...
def prepare_HierarchicalLoss(
    n_bottom_timeseries, n_bottom_timesteps, df_Sc=None, df_St=None
):
    # Create Sc
    if df_Sc is None:
        Sc = eye(n_bottom_timeseries, dtype=np.float32)
        denominator_c = np.full(
            (n_bottom_timeseries, 1), fill_value=1, dtype=np.float32
        )
    else:
    ...
    if df_St is None:
        ...
        if hasattr(df_St, "sparse"):
            ...
            n_levels_t = St.sum() // St.shape[0]
            denominator_t = 1 / (
                n_levels_t * np.maximum(np.sum(St, axis=0, keepdims=True), 1)
            )
...
def prepare_RandomHierarchicalLoss(
    n_bottom_timeseries,
    n_bottom_timesteps,
    max_levels_random,
    max_categories_per_random_level,
    rng,
):
...
def RandomHierarchicalLossObjective(
    preds,
    train_data,
    rng,
    n_bottom_timeseries,
    ...
    if number < (1 / hier_freq):
        ...
        S_aggs_list = []
        for _ in range(n_levels_random):
            n_categories_per_level = rng.integers(
                2, max_categories_per_random_level + 1
            )
            codes = rng.integers(0, n_categories_per_level, size=(n_bottom_timeseries,))
            S_agg = csc_matrix((ones, (codes, idx_range)))
            S_aggs_list.append(S_agg)
